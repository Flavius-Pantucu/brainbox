...
### The mark
The BrainBox mark is a brain, filled rather than outlined: two mirrored halves of one silhouette path in `var(--signal)`, with the hemisphere division and four folds cut out of the fill by an SVG mask rather than drawn over it. Cutting instead of overdrawing is what lets the mark sit on either coat and on any ground without a second colour — the ground shows through the folds. The solid silhouette is a legibility finding, not a style preference: four earlier drafts were drawn and rendered at icon sizes before being dropped — a brain inside a box (the box ate the brain below 32px), a single folded path (read as the digit 2), a two-tone split (the light half vanished on the day coat), and folds routed as circuit traces (read as a plus sign below 32px). It ships in `components/board/logo.js` as `BrainBoxMark`, sits in the rail at 30px via `.rail__logo`, and is duplicated as static favicons in `app/icon.svg` (the bare mark on transparency) and `app/apple-icon.png` (the mark on a navy tile, rasterised at 180x180 because Next accepts only jpg, jpeg and png for an apple icon), with the paints hardcoded because a static SVG cannot read CSS variables. The mark and the wordmark travel together in `.rail__mark` and nothing sits between or beneath them.
