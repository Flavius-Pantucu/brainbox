---
name: GameHub
description: A hand-operated club board, relit in navy and cobalt, where every number is a plate somebody hung.
colors:
  paint-navy: "#0c1725"
  paint-slate: "#24384f"
  paint-steel: "#8fa2b8"
...
typography:
  ...
  stage-title:
    ...
    letterSpacing: "-0.01em"
  wordmark:
    fontFamily: "Big Shoulders Stencil, ui-sans-serif, sans-serif"
...
GameHub is a hand-operated board in a club back room, now painted in navy and cobalt. Nothing on it is generated: every figure sits on an enamel plate that somebody screwed to a hook, every region is a channel routed into the panel, every label is stencilled on. The interface reads as fabricated hardware rather than rendered software — plates catch a hard offset shadow, slots sink into an inset channel, and cobalt appears only where a fitting would be metal.
...
- Stencil for what the board painted on itself; condensed display for figures; Archivo for anything a person wrote
- State is a stencilled word plus a drawn mark, never color alone
...
### Neutral
- **Board Navy** (`--paint-navy`): the night ground, the plate ink on enamel, the heavy 2px sudoku box rules, and the base of every routed mix.
...
- **Bone** (`--paint-bone`): the night ink and the day ground. All keylines are bone at low alpha on the night coat.
...
**Display Font:** Big Shoulders (self-hosted, `--font-display`)
**Body Font:** Archivo (self-hosted, `--font-text`)
**Label Font:** Big Shoulders Stencil (self-hosted, `--font-stencil`)

**Character:** Three faces doing three jobs. Big Shoulders is compressed industrial signage and carries every figure. Its stencil cut is what the board painted on itself: wordmark, zone labels, tags, button faces, table headers. Archivo is the only humanist voice and handles everything a person would have written by hand.

### Hierarchy
- **Display** (800, `clamp(2.6rem, 6vw, 4.2rem)`, 0.88): the day's game on the oversized dashboard plate. One per screen.
- **Headline** (800, up to `clamp(2rem, 5vw, 3.2rem)`, 0.9): the play title and the game-over verdict on a stage veil. The end-of-game plate steps down to `clamp(1.9rem, 5vw, 2.4rem)` inside a plate body.
- **Numeral wide** (800, `clamp(2.6rem, 7vw, 3.6rem)`, 0.86): the streak count on the dashboard — the single largest figure the board shows.
- **Numeral** (800, `clamp(1.9rem, 4.5vw, 2.6rem)`, 0.9): personal-best values on plates. Drops to a flat `1.45rem` under 720px.
- **Stage title** (800, `clamp(1.4rem, 2.6vw, 2rem)`, 1): the game name in the stage bar. Deliberately smaller than the dashboard display — inside a game, the board yields to the board.
- **Fixed figures** (800, `1.1rem`–`1.7rem`): readout values and day plates at `1.5rem`, room code at `1.7rem`, ladder score at `1.15rem`, seat mark at `1.2rem`, number-pad digits at `clamp(1.4rem, 2.4vw, 1.9rem)`.
- **Cell figures** (700, `5.4cqmin`; notes `1.9cqmin`): sudoku digits size to their own container, not to the viewport, so the grid stays typographically correct at any board size.
- **Wordmark** (stencil 700, `clamp(1.35rem, 2.4vw, 1.75rem)`, `0.16em`): the rail mark only.
- **Title** (stencil 700, `0.86rem`, `0.14em`, uppercase): game names on slot plates; ladder names at `0.76rem`/`0.16em`.
...
- **Label** (stencil 600, `0.72rem`, `0.24em`, uppercase): zone labels.
- **Micro** (stencil 600, `0.62rem`–`0.68rem`, `0.16em`–`0.2em`, uppercase): tags, table headers, foot, peg options, key faces (`0.82rem`, `0.2em`; large key `0.95rem`), tools (`0.64rem`).
- **Nano** (stencil 600, `0.56rem`, `0.18em`–`0.22em`, uppercase): readout labels, field labels, room-code label, the divider rule.

### Named Rules
**The Three Faces Rule.** Archivo, Big Shoulders, Big Shoulders Stencil — all self-hosted, no fourth face, and no system display stack. A new weight is allowed; a new family is a system change.

**The Stencil-For-Labels Rule.** Stencil is what the board painted on itself: wordmark, zone labels, tags, button faces, column headers, unit lines. It is always uppercase, always letterspaced ≥ `0.12em`, and never sets a sentence.
...
**The Every-Zone-Names-Itself Rule.** Every region of the board opens with a stencilled zone label whose trailing hairline runs to the edge of the region. Wayfinding is reading, not iconography.
...
Corners are barely there and hardware-derived: 3px on plates and the number-pad key, 4px on routed slots, 2px on fittings (keys, tags, lamp, nameplate, sudoku board), 3px on controls and fields, 6px on the tic-tac-toe cell, and full pills only on the peg groove and the scrollbar thumb. Nothing is round for softness; the pill exists because a peg slides in a groove.

Edges are stated, not implied. Plates carry a 1px dark border plus a second painted enamel keyline inset 4px inside the edge (6px on the play surface). The sudoku board draws its structure at two weights: 1px cell rules at 22% navy, 2px navy rules on box edges and the outer frame. Tic-tac-toe refuses a box entirely — the board is four drawn SVG hash strokes in steel-over-ground at 1.1 stroke width, with no cell borders at all.
...
- **Shape:** near-square fitting (2px radius) with a 1px dark border.
- **Primary (`.key`):** cobalt face, `--signal-on` label, stencil 700 at `0.82rem`/`0.2em` uppercase, 14px × 26px padding, sitting on a 3px hard shadow.
...
### Chips — the tag
State is a stencilled word plus a drawn 7px mark inside a `currentColor` 1px border.
...
- **Plate (`.plate`):** raised enamel — plate face, 3px radius, 1px dark border, inset enamel keyline, `--lift-1`, 20px body padding (26/28/24 on `.plate--tall`). Empty variants are honest: `.plate--empty` is a dashed outline with no face; `.plate--notch` is the routed channel with the cut shadow, showing an open hook rather than a ghost plate.
- **Slot (`.slot`):** cut channel — routed color, 4px radius, `--cut`, 16px padding (20px deep). Everything that reads as recessed (`.readout`, `.status`, `.field__input`, the peg groove) uses the same channel treatment.

### Inputs / Fields
- **Style:** `.field__input` is a routed channel — routed background, strong keyline border, 3px radius, inset cut shadow, inherited body font, 9px × 12px padding. The label above it is nano stencil.
- **Focus:** the border goes cobalt; the outline is suppressed in favor of it. Everywhere else, focus is the global 2px cobalt ring at 3px offset.
- **Code variant (`--code`):** display 800 at `1.5rem`, `0.3em` tracking, centered and uppercased — a code being read aloud.
...
### Navigation — the rail
Sticky, ground-colored, strong keyline beneath, seated with the rail shadow. Links are stencil 600 at `0.76rem`/`0.18em` uppercase in quiet ink, going full ink on hover. The current page is marked by a 3px cobalt underline sitting on the rail's bottom edge with its own 1px hard shadow — the one place cobalt fills a bar. Under 720px the rail shrinks to 60px, the mark subtitle and nameplate drop out, and links tighten to `0.68rem`/`0.12em`.
...
- **`.readout` / `.readout__item`** — a three-up cut channel of nano stencil label over a `1.5rem` display figure; `.is-warn` turns the figure to `--signal-ink`.
...
- **`.tools` / `.tool`** — a 2-column (3 under 900px) grid of quiet stencil toggles with a keyline border; `.is-on` fills 20% cobalt and turns border and ink up. The shortcut hint inside is `--signal-ink`.
...
### Sudoku board
A container-query grid: `container-type: size` on the board, so cell type is sized in `cqmin` and the grid is correct at any dimension. Enamel face, 2px navy frame, 1px 22%-navy cell rules with 2px navy box rules on the third edges. Highlight steps escalate in one hue: peer 8% navy, match 30% cobalt, selected 46% cobalt. Givens are navy 800; entries are `--signal-ink`; a wrong entry is fault red on a 16% fault wash (30% when selected). Pencil notes are a 3×3 sub-grid at `1.9cqmin`. `.sud__veil` covers the board for dealing, paused, won and lost with an 88% ground wash, a 3px blur, a stencil line and a display verdict.

### Tic-tac-toe board
No box. Four SVG hash strokes in steel-mixed-with-ground at 1.1 width sit above a bare 3×3 grid; cells are transparent with a 6px radius and only appear on interaction — hover washes 16% cobalt and reveals a ghost mark at 26% ink, a winning cell holds a 22% cobalt wash. X is drawn in `--ink`, O in `--signal`, both stroked at width 9 with the `draw-mark` dash reveal; the winning line strikes through in `--signal-ink`.
...
- **Do** open every region with a stencilled zone label.
...
- **Don't** add a fourth font family or fall back to a system display face.
