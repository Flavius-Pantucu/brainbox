...
// The board as a plain square → piece map, which is what an editor can push
// pieces around in.
export function placementOf(board) {
  const map = {};
  for (const row of board) {
    for (const piece of row) {
      if (piece) map[piece.square] = { type: piece.type, color: piece.color };
    }
  }
  return map;
}

// Castling rights are read off the placement rather than asked for: a rook and
// king still on their squares can still castle, and nothing else can.
export function fenFrom(map, turn = "w") {
  const rows = RANKS.map((rank) => {
    ...
    return gap ? row + gap : row;
  });

  const at = (square, type, color) =>
    map[square]?.type === type && map[square]?.color === color;

  let castling = "";
  if (at("e1", "k", "w")) {
    if (at("h1", "r", "w")) castling += "K";
    if (at("a1", "r", "w")) castling += "Q";
  }
  if (at("e8", "k", "b")) {
    if (at("h8", "r", "b")) castling += "k";
    if (at("a8", "r", "b")) castling += "q";
  }

  return `${rows.join("/")} ${turn} ${castling || "-"} - 0 1`;
}

// What is wrong with a position someone has set up, in words, or null.
export function placementProblem(map) {
  const kings = { w: 0, b: 0 };
  for (const [square, piece] of Object.entries(map)) {
