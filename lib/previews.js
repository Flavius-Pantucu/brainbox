...
// A table part way through: two players open, and a rack with a run and a group
// bracketed on it, loose tiles between them.
function racked(pairs) {
  const slots = new Array(26).fill(null);
  for (const [at, tile] of pairs) slots[at] = tile;
  return slots;
}

export const REMI_PREVIEW = {
  seats: ["a", "b", "c"],
  names: { a: "You", b: "Machine one", c: "Machine two" },
  you: "a",
  turn: "a",
  table: [
    { seat: "b", tiles: [10, 11, 12] },
    { seat: "c", tiles: [16, 29, 42] },
  ],
  held: { a: 10, b: 11, c: 13 },
  opened: { a: false, b: true, c: true },
  scores: { a: 0, b: 24, c: 41 },
  slots: racked([
    [0, 4], [1, 5], [2, 6],
    [4, 0], [5, 27],
    [7, 21], [8, 34], [9, 47],
