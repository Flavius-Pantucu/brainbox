...
export const play = pgTable(
  "play",
  {
    // Supplied by the client, and unique only WITHIN a player. It was the
    // primary key on its own to begin with, which meant two players who ever
    // generated the same id silently lost a game each — the second insert hit
    // onConflictDoNothing and vanished with a 200. An id a browser chooses can
    // never be a global key.
    id: text("id").notNull(),
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
  ...
  (t) => [
    // the id is the player's, so the key is both
